...
def plot_clean_accuracy(results: Dict[str, Dict[str, Dict[str, float]]], fig_dir: Path) -> Path:
    """
    results[method][occ]["acc"]
    """
    ...
    accs = [results[m]["none"]["acc"] for m in methods]
    labels = [_pretty_method(m) for m in methods]

    fig_dir.mkdir(parents=True, exist_ok=True)
    out_path = fig_dir / "clean_accuracy.png"
    ...
    plt.ylim(0.3, 0.7)
    plt.ylabel("Accuracy")
    plt.title("Clean Accuracy (UTKFace 7-bin)")
...
def plot_robustness_accuracy(results: Dict[str, Dict[str, Dict[str, float]]], fig_dir: Path) -> Path:
    """
    Line plot: Accuracy vs occlusion type for each method.
    """
    ...
    fig_dir.mkdir(parents=True, exist_ok=True)
    out_path = fig_dir / "robustness_accuracy.png"
    ...
    plt.ylim(0.3, 0.7)
    plt.ylabel("Accuracy")
    plt.title("Accuracy Under Structured Occlusion")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
...
def plot_robustness_drop(results: Dict[str, Dict[str, Dict[str, float]]], fig_dir: Path) -> Path:
    """
    ...
    for i, m in enumerate(methods):
        clean = results[m]["none"]["acc"]
        for j, occ in enumerate(occs):
            if occ in results[m]:
                drops[i, j] = clean - results[m][occ]["acc"]
            else:
                drops[i, j] = np.nan

    fig_dir.mkdir(parents=True, exist_ok=True)
    out_path = fig_dir / "robustness_drop.png"
    ...
    plt.ylabel("Accuracy Drop (Clean - Occluded)")
    plt.title("Occlusion Sensitivity (Absolute Drop from Clean)")
    plt.grid(axis="y", alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
...
def plot_confusion_matrix(cm: np.ndarray, title: str, out_path: Path, class_names: List[str] | None = None) -> None:
...
def main() -> None:
    ...
    p1 = plot_clean_accuracy(results, FIG_DIR)
    print(f"Saved: {p1}")

    p2 = plot_robustness_accuracy(results, FIG_DIR)
    print(f"Saved: {p2}")

    p3 = plot_robustness_drop(results, FIG_DIR)
    print(f"Saved: {p3}")

    # Confusion matrix for CNN clean, if present
    if "cnn" in raw_by_method:
        cm = _extract_confmat(raw_by_method["cnn"], occ="none")
        if cm is not None:
            # Optional class labels from bins
            ...
            if isinstance(bins, list) and len(bins) >= 2:
                ...
                try:
                    ...
                    for i in range(len(b) - 1):
                        lo, hi = b[i], b[i + 1]
                        if i == len(b) - 2:
                            class_names.append(f"{lo}+")
                        else:
                            class_names.append(f"{lo}–{hi-1}")
                except Exception:
                    class_names = None
